...
$.fn.addtabs = function (options) {
    ...
    Addtabs.options = $.extend({
        ...
        iframeHeight: tabContentHeight, //固定TAB中IFRAME高度,根据需要自己修改
        method: 'init',
    ...
    obj.on('click', 'ul.rightMenu a[data-right=remove-circle]', function () {
        ...
        obj.children('ul.nav').find('li[id!="tab_tab_home"]').each(function () {
            var id = $(this).attr('id');
    ...
    obj.on('click', 'ul.rightMenu a[data-right=remove-left]', function () {
        var tab_id = $(this).parent('ul').attr("aria-controls");
        $('#tab_' + tab_id).prevUntil().not('li[id=tab_tab_home]').each(function () {
            var id = $(this).attr('id');
            if (id && id != 'tab_' + tab_id) {
                Addtabs.close($(this).children('a').attr('aria-controls'));
            }
...
window.Addtabs = {
    ...
    add: function (opts) {
        ...
        Addtabs.pop(id,$('#tab_' + id).attr("aria-url"),$('#tab_' + id));


    },
    pop: function (id,url,e) {

        $('body').find('#popMenu').remove();
        if (typeof(id) == "undefined"){
            return;
        }
        var pop = $('<ul>', {'aria-controls': id, 'aria-url':url,'class': 'rightMenu list-group', id: 'popMenu'})
            .append(
        ...
        pop.css({
            'top': e.context.offsetHeight + 5 + 'px',
            'left': e.context.offsetLeft  + 'px'
        });
        if(e.context.id != null){
    ...
    close: function (id) {
        //首先判断是否位于下拉框中
        var parentClass=$("#tab_" + id).parent().attr("class");
        var len=$("#tab_" + id).parent().find("li").length;
        //位于下拉菜单中的标签关闭处理
        if(parentClass=="dropdown-menu"){
            if(len==1){
                $("#tab_" + id).parent().parent().parent().find("li").last().addClass('active');
                $("#" + id).prev().addClass('active');
            }else{
                //如果关闭的是当前激活的TAB，再判断他的位置是不是下拉菜单第一个
                if (obj.find(".dropdown-menu").find("li.active").attr('id') === "tab_" + id) {
                    if($("#tab_" + id).index()==0){
                        $("#tab_" + id).next().addClass('active');
                        $("#" + id).prev().addClass('active');
                    }else{
                        var cid=$(".dropdown-menu li").eq(0).attr("id").substring(4);
    ...
    closeAll: function () {
        $.each(obj.find('li[id!="tab_tab_home"]'), function () {
            var id = $(this).children('a').attr('aria-controls');
            $("#tab_" + id).remove();
            $("#" + id).remove();
        });
    ...
    drop: function () {
        ...
        element.append(dropdown.find('li'))
            ...
            .not('.tabdrop')
            .each(function () {
                var restWidth=element.width() - $(this).position().left - $(this).width() ;
                //alert(restWidth+"-"+$(this).width());
                if (this.offsetTop > 25 || restWidth< $(this).width()) {
